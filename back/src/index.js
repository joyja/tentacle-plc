...
try {
  ...
  const runtimeConfigFile = path.resolve(runtimeDir, 'config.json')
  ...
  const config = JSON.parse(fs.readFileSync(runtimeConfigFile))

  const runtimeVariableFile = path.resolve(runtimeDir, 'variables.json')
  if (!fs.existsSync(runtimeVariableFile)) {
    fs.writeFileSync(runtimeVariableFile, JSON.stringify({}, null, 2))
  }
  const variables = JSON.parse(fs.readFileSync(runtimeVariableFile))

  const runtimeClassesDir = path.resolve(runtimeDir, 'classes')
  if (!fs.existsSync(runtimeClassesDir)) {
    fs.mkdirSync(runtimeClassesDir)
  }
  ...
  const runtimeProgramsDir = path.resolve(runtimeDir, 'programs')
  if (!fs.existsSync(runtimeProgramsDir)) {
    fs.mkdirSync(runtimeProgramsDir)
  }
  const app = express()
  app.use(cors())
    ...
    try {
      ...
      Object.keys(variables).forEach((variableKey) => {
        ...
        if (variable.datatype === 'number') {
          ...
          classes.map((item) => item.name).includes(variable.datatype)
        ) {
          variableClass = classes.find(
